...
/**
 * Builder for HNSW++ graph. See HNSW++ paper.
 */
public class HnswPlusGraphBuilder implements HnswBuilder {
    ...
    /**
     * Default LID threshold value
     */
    private static final Double LID_THRESHOLD = 20d;
    ...
    private final Map<Integer, Double> lidValues = new HashMap<>();

    private final Map<Integer, StreamingKmeans> perLayerCentroids = new HashMap<>();

    public static HnswPlusGraphBuilder create(
            RandomVectorScorerSupplier scorerSupplier, int M, int beamWidth, long seed)
            throws IOException {
        return new HnswPlusGraphBuilder(scorerSupplier, M, beamWidth, seed, -1);
    }

    public static HnswPlusGraphBuilder create(
            RandomVectorScorerSupplier scorerSupplier, int M, int beamWidth, long seed, int graphSize)
            throws IOException {
        return new HnswPlusGraphBuilder(scorerSupplier, M, beamWidth, seed, graphSize);
    }
     ...
     */
    protected HnswPlusGraphBuilder(
            RandomVectorScorerSupplier scorerSupplier, int M, int beamWidth, long seed, int graphSize)
            throws IOException {
...
/*
  private void linkNode(int node, int level, UpdateableRandomVectorScorer scorer, int... neighbors) throws IOException {
    // Existing connection logic
    for (int neighbor : neighbors) {
      //graph.addEdge(node, neighbor, level);
      NeighborArray nbrsOfNbr = hnsw.getNeighbors(level, neighbor);
      nbrsOfNbr.addAndEnsureDiversity(node, scorer.score(neighbor), neighbor, scorer);
    }

    // Add skip bridges for high-LID nodes
    if (lidValues.getOrDefault(node, 0.0) > LID_THRESHOLD) {
      int[] distantNodes = findDistantNeighbors(node, scorer);
      for (int distant : distantNodes) {
        graph.addEdge(node, distant.node, level);
      }
    }
  }

  private List<Neighbor> findDistantNeighbors(int node, UpdateableRandomVectorScorer scorer) throws IOException {
    scorer.setScoringOrdinal(node);
    PriorityQueue<Neighbor> distantNeighbors = new PriorityQueue<>(Comparator.comparingDouble);

    for (int otherNode : hnsw.getAllNodes()) {
      if (!hnsw.getNeighbors()isConnected(node, otherNode)) {
        double score = scorer.score(otherNode);
        distantNeighbors.add(new Neighbor(otherNode, score));
      }
    }

    List<Neighbor> result = new ArrayList<>();
    while (!distantNeighbors.isEmpty() && result.size() < BRIDGE_COUNT) {
      result.add(distantNeighbors.poll());
    }
    return result;
  }

  private List<Integer> searchLayer(int entryPoint, int ef) {
    PriorityQueue<Neighbor> candidates = new PriorityQueue<>();
    candidates.add(new Neighbor(entryPoint, scorer.score(entryPoint, query)));

    Set<Integer> visited = new HashSet<>();
    List<Integer> result = new ArrayList<>();

    while (!candidates.isEmpty()) {
      Neighbor top = candidates.poll();
      if (visited.contains(top.node)) continue;
      visited.add(top.node);
      result.add(top.node);

      // Expand to neighbors
      for (Neighbor neighbor : graph.getNeighbors(top.node)) {
        if (!visited.contains(neighbor.node)) {
          candidates.add(neighbor);
        }
      }

      // If the node has a skip bridge, explore it as well
      if (lidValues.getOrDefault(top.node, 0.0) > LID_THRESHOLD) {
        for (Neighbor bridge : graph.getSkipBridges(top.node)) {
          if (!visited.contains(bridge.node)) {
...
*/
    ...
    private void addNodeLayer(int layer, int node, UpdateableRandomVectorScorer scorer) throws IOException {
        int[] eps = getCentroids(layer, node, scorer);
        //hnsw.addNode(layer, node);
        double lid;
        if (!lidValues.containsKey(node)) {
            // compute LID over its neighbors
            HnswPlusGraphBuilder.GraphBuilderKnnCollector lidNeighbors = graphSearcher.searchLevel(scorer, (int) Math.sqrt(hnsw.size()), layer, eps, hnsw);
        ...
        if (lid > LID_THRESHOLD) {
            efConstruction *= 2;  // Increase ef for high-LID nodes
        }

        // get the neighbors to be connected on layer 0
        HnswPlusGraphBuilder.GraphBuilderKnnCollector neighborsCollector = graphSearcher.searchLevel(scorer, efConstruction, layer, eps, hnsw);
        neighborsCollector.popUntilNearestKNodes();
        NeighborArray zeroNeighbors = new NeighborArray(Math.max(neighborsCollector.k(), efConstruction + 1), false);
        int noNeighbors = neighborsCollector.size();
        // extract all the Neighbors from the queue into an array; these will now be sorted from worst to best
        for (int i = 0; i < noNeighbors; i++) {
            float maxSimilarity = neighborsCollector.minimumScore();
            zeroNeighbors.addInOrder(neighborsCollector.popNode(), maxSimilarity);
        }
        // link nodes to its neighbors at zero-th layer
        addDiverseNeighbors(layer, node, zeroNeighbors, scorer);
    }

    public void addGraphNode(int node, UpdateableRandomVectorScorer scorer) throws IOException {
        //  prioritize high-LID nodes and adjust neighbor selection
        if (frozen) {
            throw new IllegalStateException("Graph builder is already frozen");
        }

        // promote itself as entry node in bottom layer, if entry node is not set
        if (hnsw.trySetNewEntryNode(node, 0)) {
            return;
        }
        ...
        // find nearest neighbors at layer 0 to calculate LID, with a ef_construct = sqrt(|graph|)
        int layer = 0;
        scorer.setScoringOrdinal(node);
        ...
        if (!lidValues.containsKey(node)) {
            // compute LID over its neighbors
            HnswPlusGraphBuilder.GraphBuilderKnnCollector lidNeighbors = graphSearcher.searchLevel(scorer, Math.max(1, (int) Math.sqrt(hnsw.size())), 0, eps, hnsw);
            lid = computeLID(node, lidNeighbors.popUntilNearestKNodes());
            lidValues.put(node, lid);
        } else {
            lid = lidValues.get(node);
        ...
        // get the neighbors to be connected on layer 0
        HnswPlusGraphBuilder.GraphBuilderKnnCollector neighborsCollector = graphSearcher.searchLevel(scorer, efConstruction, 0, eps, hnsw);
        neighborsCollector.popUntilNearestKNodes();
        NeighborArray zeroNeighbors = new NeighborArray(Math.max(neighborsCollector.k(), efConstruction + 1), false);
        int noNeighbors = neighborsCollector.size();
        // extract all the Neighbors from the queue into an array; these will now be sorted from worst to best
        ...
        // do the same at above layers, with decreasing probability
        double nodeInsertionProbability = 1d;
        boolean add = true;
        for (int level = 1; level < nodeLevel ; level++) {
            //if (add && Math.random() < nodeInsertionProbability) {
                // insert node at this layer
                addNodeLayer(level, node, scorer);
            //} else {
            //    add = false;
            //}
            //nodeInsertionProbability /= 2; // decrease probability
        }
/*
    // then add nodes to the above levels, by nearest centroid
    for (int level = hnsw.numLevels() - 1; level > 0; level--) {
      // not this, of course
      hnsw.addNode(level, node);
    }

    // now how do I distribute it over the layers ???
   ...
   */
    ...
    private int[] getCentroids(int layer, int node, UpdateableRandomVectorScorer scorer) throws IOException {
        ...
        int closestNode = layerCentroids.update(node);
        return new int[] {closestNode};
    }
    ...
    */
    ...
    private boolean[] selectAndLinkDiverse(
        ...
        // Modify this to include LID-driven bridge creation.
    ...
    private void centroids() {
    /*
    for (int level = hnsw.numLevels() - 1; level > 0; level--) {
      int[] centroids = fitCentroids(level);
    }
     */
    }

    private int[] fitCentroids(int level) {
    /*int nClusters = 20;
    int nVectors = 10;
    ...
    private static final class StreamingKmeans {
        ...
        void initCentroids() throws IOException {
            initializeForgy();
        }
        ...
        */
        int update(int node) throws IOException {
            ...
            int closestIndex = argmin(distances);
            ...
            return closestNodes[closestIndex];
        }

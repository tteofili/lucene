...
public class HnswGraphBuilder implements HnswBuilder {
  ...
  public void addGraphNode(int node) throws IOException {
    ...
    */
    ...
    do {
      ...
      RandomAccessVectorValues<float[]> values = ((RandomVectorScorerSupplier.FloatScoringSupplier) scorerSupplier).vectors;
      ...
      for (int i = scratchPerLevel.length - 1; i >= 0; i--) {
        ...
        float[] newVector = (float[]) values.vectorValue(node);
        float[] residual = new float[newVector.length];
        for (int neighborNode : eps) {
          float[] neighbor = (float[]) values.vectorValue(neighborNode);
          /*
  ...
  private void addDiverseAndOrthogonalNeighbors(RandomAccessVectorValues values, int level, int node, NeighborArray candidates)
          throws IOException {
  ...
  private boolean[] selectAndLinkDiverseAndOrthogonal(RandomAccessVectorValues values,
    ...
    for (int i = candidates.size() - 1; neighbors.size() < maxConnOnLevel && i >= 0; i--) {
      ...
      float[] candidateResidual = candidates.residuals()[i];
    ...
    infoStream.message(HNSW_COMPONENT, orthogonals+":"+diverse);
    return mask;
  }
